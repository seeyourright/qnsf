  ...
  <div style="margin-top: 20px">
    <el-form ref="form" class="form" label-width="150px" :rules="rules" :model="form">
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item label="鉴定所名称" prop="lawFirmName">
      ...
      <el-form-item label="鉴定所地址" prop="phone">
        <el-input type="textarea" v-model="form.address"></el-input>
      </el-form-item>
      <el-form-item label="鉴定所简介" prop="phone">
...
<script>
  export default {
    ...
    created () {
      if (this.$store.state.user.userType === '2' && this.$store.state.user.unitId !== '5227000000' || this.$store.state.user.userType === '6') {
        this.allper = false
      }
      if (this.$parent.id) {
        this.init()
    ...
    methods: {
      init () {
        this.$http.get(this.$url.Appraisal_Office_By_Id, {id: this.$parent.id}).then(res => {
          if (res.code === 200) {
            this.form = res.data
      ...
      async edit () {
        ...
        this.$http.post(this.$url.Update_Appraisal_Office, params).then(res => {
          if (res.code === 200) {
