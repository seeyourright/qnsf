  ...
  <div class="mm">
    <div class="condition">
      ...
      <el-form size="small" inline>
        ...
        <el-form-item>
          <el-input placeholder="输入律所名称" v-model="condition.lawFirmName"></el-input>
        </el-form-item>
        <el-form-item>
  ...
  export default {
    ...
    created () {
      if (this.$store.state.user.userType === '4') {
        this.$router.push('lsDetail/lsBase')
      }
    },
    mounted () {
    ...
    methods: {
      getData (page) {
        ...
        this.$http.get(this.$url.Law_Firm_List, {page, limit: this.size, ...this.condition}).then(res => {
          if (res.code === 200) {
      ...
      addHandler () {
        this.$router.push('lawyerServicesAdd')
      },
      detailHandler (row) {
        this.$router.push('lsDetail/lsBase?id=' + row.id)
      },
      deleteAllHandler () {
        ...
        this.$confirm('确定删除吗').then(() => {
          this.$http.post(this.$url.Delete_Law_Firm, {ids: ids.join(',')}).then(res => {
            if (res.code === 200) {
