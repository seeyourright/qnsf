<template>
  <div style="font-size: 14px;width: 800px">
    ...
    <div class="item">
      ...
      <div style="flex: 1">
        <ul class="filelist">
          <li v-for="item in form.lawyerFilesList"><a href="javascript:;" @click="download(item)"><i class="iconfont icon-wenjian"></i>{{item.fileUrl|urlName}}</a></li>
        </ul>
      </div>
      <div>
        <a style="color: #8cc5ff" :href="$url.Law_Order_Download_file +'?id=' + id" target="_blank">下载全部</a>
      </div>
    </div>
    <div class="item">
      <div>委托人签名：</div>
    ...
    <div class="item">
      <div style="line-height: 40px">办理律师：</div>
      <el-select :disabled="form.status!==0" v-model="lawyer">
  ...
  export default {
    ...
    methods: {
      ...
      refuse () {
        ...
        this.$http.post(this.$url.Update_Law_Order, params).then(res => {
          if (res.code === 200) {
