<template>
  <div style="font-size: 14px;width: 800px">
    <el-row class="info">
      ...
      <el-col :span="8">委托人：{{form.refereeName}}</el-col>
      <el-col :span="8">联系电话：{{form.refereePhone}}</el-col>
      <el-col :span="8">业务类型：{{form.businessType}}</el-col>
      <el-col v-if="form.status === 2" :span="8">拒绝时间：{{form.updateTime}}</el-col>
      <el-col v-if="form.status === 1" :span="8">通过时间：{{form.updateTime}}</el-col>
    </el-row>
    <div class="item">
      <div>相关证明材料：</div>
      <div style="flex: 1">
        <ul class="filelist">
          <li v-for="item in form.fileUrls"><a href="javascript:;" @click="download(item)" target="_blank"><i class="iconfont icon-wenjian"></i>{{item.fileUrl|urlName}}</a></li>
        </ul>
      </div>
      <div>
        <a style="color: #8cc5ff" :href="$url.Notarial_Order_Download_file +'?orderId=' + id" target="_blank">下载全部</a>
      </div>
    </div>
    <div class="item">
      <div>委托人签名：</div>
      <div class="sign"><img :src="form.signatureAddress" alt=""></div>
    </div>
...
<script>
  export default {
    ...
    methods: {
      init () {
        this.$http.get(this.$url.Notarial_Order_By_Id, {id: this.id}).then(res => {
          if (res.code === 200) {
            this.form = res.data
            this.refuseReason = this.form.refusalCause
          }
        })
      },
      download (item) {
      ...
      refuse () {
        ...
        this.$http.post(this.$url.Update_Notarial_Order, params).then(res => {
          if (res.code === 200) {
