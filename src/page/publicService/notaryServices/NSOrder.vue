  ...
  export default {
    ...
    methods: {
      getData (page) {
        ...
        this.$http.get(this.$url.Notarial_Order_List, {page, limit: this.size, GzcId: this.$parent.id, ...this.condition}).then(res => {
          if (res.code === 200) {
      ...
      detailHandler (row) {
        this.$router.push('nsOrderApproval?id=' + this.$parent.id + '&zid=' + row.id)
      },
      deleteAllHandler () {
        ...
        this.$confirm('确定删除吗').then(() => {
          this.$http.post(this.$url.Delete_Notarial_Order, {idList: ids}).then(res => {
            if (res.code === 200) {
