  ...
  <div class="mm">
    <div class="condition">
      ...
      <el-form size="small" inline>
        ...
        <el-form-item>
          <el-input placeholder="输入公证处名称" v-model="condition.name"></el-input>
        </el-form-item>
        <el-form-item>
  ...
  export default {
    ...
    created () {
      if (this.$store.state.user.userType === '5') {
        this.$router.push('nsDetail/nsBase')
      }
    },
    mounted () {
    ...
    methods: {
      getData (page) {
        ...
        this.$http.get(this.$url.Notarial_Office_List, {page, limit: this.size, ...this.condition}).then(res => {
          if (res.code === 200) {
      ...
      addHandler () {
        this.$router.push('notaryServicesAdd')
      },
      detailHandler (row) {
        this.$router.push('nsDetail/nsBase?id=' + row.id)
      },
      deleteAllHandler () {
        ...
        this.$confirm('确定删除吗').then(() => {
          this.$http.post(this.$url.Delete_Notarial_Office, {idList: ids}).then(res => {
            if (res.code === 200) {
